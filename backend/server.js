...
function validateFiles(fileMap) {
  ...
  for (const [key, rule] of Object.entries(FIELD_RULES)) {
    ...
    if (!rule.ext.includes(ext)) {
      errors.push(`${rule.label} 확장자 오류: ${ext}. 허용 확장자: ${rule.ext.join(", ")}`);
    }
...
function buildSubmission(body, receiptId) {
  ...
  try {
    workSteps = JSON.parse(body.workStepsJson || "[]");
  } catch (_) {
    workSteps = [];
  }
...
async function createSubmissionWorkbook(submission, fileMap) {
  ...
  s1.addRow([
    ...
    submission.handlingMaterials,
    submission.handlingProcess,
    submission.contractStart.replaceAll("-", ""),
    submission.contractEnd.replaceAll("-", "")
  ]);
  s1.columns = [
    { width: 12 },
    { width: 34 },
  ...
  s2.addRow([
    ...
    submission.contractTitle,
    submission.handlingProcess,
    `${dotDate(submission.contractStart)}~${dotDate(submission.contractEnd)}`,
    submission.workerCount
  ]);
  s2.columns = [
    { width: 8 },
    { width: 22 },
  ...
  s3.addRow([
    submission.companyName,
    submission.safetyManagerName,
    submission.safetyManagerPosition,
    submission.safetyManagerPhone,
  ...
  for (const step of submission.workSteps || []) {
    s4.addRow([
      `${step.step}단계`,
      step.title || "",
      step.detail || ""
  ...
  for (const [key, rule] of Object.entries(FIELD_RULES)) {
    ...
    s5.addRow([
      ...
      `${rule.ext.join(", ")} / ${rule.maxMB}MB 이하`,
      key === "pledge" ? "자동연장 조항 해당 시" : ""
    ]);
  }
  ...
  try {
    ...
    const baseAttachments = [
      ...
      ...baseFileKeys
        ...
        .map((key) => makeAttachment(fileMap[key]))
    ];

    const trainingAttachments = fileMap.trainingCertificate
      ? [makeAttachment(fileMap.trainingCertificate)]
      : [];
    ...
    if (!sendgridApiKey) {
      throw new Error("SENDGRID_API_KEY 환경변수가 없습니다.");
    }
